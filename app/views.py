...
#from django.core.management.commands import create
...
def blog(request):
    ...
    posts = Blog.objects.all()[:3]    # топ 3
    one = Blog.objects.last()
    #order_by('-posted')        # запрос на выбор всех статей из модели, отсортированных по убыванию даты опубликования 
...
def blogpost(request, parametr):
    ...
    number = Comment.objects.filter(post=parametr).count()  #подсчет количества комментариев
...
#def your_choice(request, parametr):
#    """Renders the blogpost page."""
#    post_1 = Blog.objects.get(id=parametr)      # запрос на выбор конкретной статьи по параметру
#    zakaz = Basket()
#    zakaz.title_b=post_1.title
#    zakaz.description_b=post_1.description 
#    zakaz.content_b=post_1.content
#    zakaz.kol=5
#    zakaz.save()
...
#            return redirect('your_choice', parametr=post_1.id)     # переадресация на ту же страницу статьи после отправки комментария
#    else:                                           
#        form = CommentForm()                                 # создание формы для ввода комментария
